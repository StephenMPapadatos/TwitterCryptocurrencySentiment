...
const searchTerms = '$TRX, $DRGN, $ETH, $LTC, $NAV, $SALT, $VEN, $XRB, $MIOTA, $BNB, $WAX, $GNT, $STEEM, $REQ, $DNT, $XRP, $WAVES, $DBC, $ICX, $MOD, $MCO, $XLM, $NEO, $ECA, $BTC';
const searchTermsArray = searchTerms.toLowerCase().split(/\s*,\s*/);

for(var i = 0; i < searchTermsArray.length; i++) {
	searchTermsArray[i] = searchTermsArray[i].substr(1);
}
...
// Uses a Firebase transaction to incremement a counter
...
tweetRef.on('value', function (snap) {
		if(snap.val() != null) {
			...
			var searchTermInTweet = new Array();
			
			for(var i = 0; i < searchTermsArray.length; i++) {
				if(tweet.text.toLowerCase().search(searchTermsArray[i]) != -1) {
					searchTermInTweet.push(searchTermsArray[i]);
				}
			}
			
			for(var i = 0; i < searchTermInTweet.length; i++) {
				var root = "coins/" + searchTermInTweet[i] + "/";
				
				let scoreRef = db.ref(root + 'score/');
				...
				for (let i in tokens) {
					...
					if (!(word.match(/[^A-Za-z0-9]/g))) {
						let posRef = db.ref(root + 'tokens/' + token.partOfSpeech.tag);
			...
			if (hashtags) {
				for (let i in hashtags) {
				  ...
				  let hashtagRef = db.ref(root + 'hashtags');
